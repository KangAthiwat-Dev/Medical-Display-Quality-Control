...
# บันทึกไฟล์ screenqc.db ไว้ใน database/data/
...
def init_db():
    with _conn() as c:
        ...
        # สร้างบัญชี Admin เริ่มต้น (ถ้ายังไม่มี admin ในระบบเลย ผู้ใช้จะได้ล็อคอินตอนแรกได้)
...
def verify_admin_login(username: str, password: str) -> bool:
    """ตรวจสอบ login สำหรับ Admin (ใช้แค่ username กับ password)"""
    init_db()
    with _conn() as c:
        row = c.execute(
...
def verify_login(display_name: str, password: str) -> dict | None:
    """ตรวจสอบ login โดย display_name = 'ชื่อ นามสกุล' -> คืนค่าข้อมูล dict ถ้าถูกต้อง"""
    init_db()
    ...
    with _conn() as c:
        row = c.execute(
            "SELECT * FROM users WHERE name=? AND lastname=? AND password=?",
            (name, lastname, password),
        ).fetchone()
        return dict(row) if row else None
...
def search_evaluations(
    ...
    sql  = """SELECT e.*,
                     (SELECT COUNT(*) FROM evaluations e2
                      WHERE e2.screen_type = e.screen_type
                        AND e2.period = e.period
                        AND e2.id <= e.id) AS rank
              FROM evaluations e
              WHERE 1=1"""
    args = []
    if hospital:
    ...
    if period:
        ...
        args.append(period)
    if date_from:
        sql += " AND substr(eval_datetime, 7, 4) || '-' || substr(eval_datetime, 4, 2) || '-' || substr(eval_datetime, 1, 2) >= ?"
