...
class SelectTypeScreen(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        ctk.CTkButton(
            self.main_frame, text=BACK_BUTTON_TEXT, font=(FONT_TH, 20, "bold"),
        ...
        # ── Title ──
        ctk.CTkLabel(
            center, text=SCREEN_TITLE_TEXT,
            font=FONT_TITLE, text_color=WHITE,
        ).pack(pady=(0, 28))

        # ── Type cards ──
        for dt in DISPLAY_TYPES:
            self._make_card(center, dt,
                            SELECT_TYPE_CARD_NORMAL, SELECT_TYPE_CARD_SELECTED, SELECT_TYPE_CARD_HOVER,
    ...
    def _make_card(self, parent, dt,
        ...
        ctk.CTkLabel(
            inner, text=dt["icon"],
            font=font_icon, text_color=WHITE, width=48,
        ...
        ctk.CTkLabel(
            text_block, text=dt["title"],
            font=font_title, text_color=WHITE, anchor="w",
        ).pack(fill="x")

        ctk.CTkLabel(
            text_block, text=dt["subtitle"],
            font=font_sub, text_color=DIVIDER_COLOR[0:2], anchor="w",
            wraplength=700, justify="left",
        ).pack(fill="x")

        # selection indicator bar (left edge, hidden by default)
        indicator = ctk.CTkFrame(card, width=6, corner_radius=3,
    ...
    def _make_period_btn(self, parent, val):
        btn = ctk.CTkButton(
            parent, text=val, font=(self._font_th, 18, "bold"),
            height=42, corner_radius=6,
            fg_color=self._card_normal, hover_color=self._card_hover,
            text_color=WHITE,
